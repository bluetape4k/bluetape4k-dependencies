...
- 판정: **BLOCKED**
- 근거 receipt: `build/issues-242-243/issue-242-receipt.json`
...
| 2. Callers/Consumers | BLOCKED | Workshop baseline의 core/Jackson/starter는 `2.2.0`으로 확인됐지만, Exposed의 benchmark/Jackson/starter는 실제 persistence runtime에서 미해석됐고 Clinic benchmark는 verification 오류로 중단됐다. |
| 3. Tests/Fixtures | BLOCKED | 중앙 runner/receipt Python 92개 targeted test는 통과했다. 필수 candidate graph와 Exposed/Workshop/Clinic consumer test는 baseline gate 실패로 실행하지 않았다. |
...
| 7. Design/Reuse/Ops Risk | BLOCKED | 공통 catalog inventory, runner, receipt validator와 cache primitive을 재사용했지만 Exposed 좌표 계약과 Clinic verification metadata의 재검토 결정이 필요하다. |
...
- Exposed: `core`만 `2.4.0`으로 선택; 나머지 세 필수 좌표는 `No dependencies matching given input`
...
P0는 없다. P1 adoption blocker는 두 가지다.

1. Exposed 네 좌표의 실제 consumer runtime 매핑을 확정한다.
